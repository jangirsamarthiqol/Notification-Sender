...
def send_notifications(title, body, tokens, batch_size=100, image_url=None, sound_url=None):
    """
    Send notifications in batches with optional custom image and sound.
        ...
        for doc_ref, token, is_array in batch:
            ...
            android_config = messaging.AndroidConfig(
                notification=messaging.AndroidNotification(
                    image=image_url,  # Custom image URL
                    sound=sound_url,  # Custom sound URL
                )
            )

            # Corrected APNSConfig (case-sensitive) and APNSPayload
            apns_config = messaging.APNSConfig(  # Correct class name
                payload=messaging.APNSPayload(  # Correct class name
                    aps=messaging.Aps(
                        alert=messaging.ApsAlert(title=title, body=body),
                        sound="default",  # Default sound for iOS (can be changed)
                    )
                )
            )

            # Create message with Android & iOS configurations
...
# Custom image and sound URLs
image_url = st.text_input("Enter the URL for the notification image (optional)")
sound_url = st.text_input("Enter the URL for the custom notification sound (optional)")
...
if send_button:
    ...
    if not cpids:
        ...
        if not tokens:
            ...
            summary, errors = send_notifications(title, body, tokens, image_url=image_url, sound_url=sound_url)
            st.write("---")
